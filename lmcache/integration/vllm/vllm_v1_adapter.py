...
class ReqMeta:
    ...
    def from_request_tracker(
        ...
        # Loading cached KV may be needed even when there are not yet enough
        # tokens in the current scheduled slice to save a full LMCache chunk.
        # In that case, keep the cached-token prefix available for retrieval
        # instead of emitting an empty token_ids payload.
        num_tokens_for_load = 0
        if load_spec is not None and load_spec.can_load:
            num_tokens_for_load = load_spec.lmcache_cached_tokens

        num_tokens_for_meta = max(num_tokens_to_save, num_tokens_for_load)
        token_ids = input_token_ids[:num_tokens_for_meta]
